...
const OperatorHTMLs = {
    ...
    [OperatorType.SuperLogarithm]: ['slg(',`+1)`],
    [OperatorType.Tetrate]: [undefined,`<img src="./textures/operator-exponent.png" draggable="false" style="width: 15"><img src="./textures/operator-exponent.png" draggable="false" style="width: 15">`],
...
const OperatorDescriptions = {
    ...
    [OperatorType.SuperLogarithm]: `Calculates the super logarithm of a variable, number, or operator.`,
    [OperatorType.Tetrate]: `Calculates the tetration with the base.`,
...
class Operator {
    ...
    run(index=-1) {
        ...
        switch (this.type) {
            ...
            break;
            case OperatorType.Tetrate:
                result = Decimal.tetrate(this.slots[0].calculate(),this.slots[1].calculate());
            break;
            case OperatorType.Logarithm:
                result = Decimal.add(this.slots[0].calculate(),1).ln();
            break;
            case OperatorType.SuperLogarithm:
                result = Decimal.add(this.slots[0].calculate(),1).slog(10, true);
            break;
...
const CustomOperatorDescriptions = new Map([
    ...
    [new Operator(OperatorType.Tetrate, null, new Slot(0, new Operator(OperatorType.Logarithm, new Slot(0, new Operator(OperatorType.SuperLogarithm), true)), true)), `${OperatorDescriptions[OperatorType.Tetrate]} The tetration equals to super logarithm of a variable, number, or operator.`],
    [new Operator(OperatorType.Repeat, new Slot(4)), `Repeats constant times that calculate inside the repeat loop.`],
])
...
function format(n, force = false) {
    const DN = D(n)
    if (DN.gte(Decimal.dLayerMax)) return "Infinite"
    if (!force && !player?.endless && DN.gt(Number.MAX_VALUE)) {return "Infinite"}
    if (DN.gte("(e^10)1")) {
   	let DNslog = DN.slog(10)
    return `<sup>${DNslog > 1000 ? format(DN.slog(10)) : (DN.slog(10)).toFixed(3)}</sup>10`;
    }
