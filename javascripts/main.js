...
(() => {
    ...
    function finish() {
        ...
        if (player.variables.P.gte(Decimal.tetrate(2,1024))) unlockAchievement(1,3);
        if (player.variables.P.gte(Decimal.tetrate(2,1048576))) unlockAchievement(1,4);
            ...
            if (x instanceof Operator) {
                if (x.type < 10) {
                    ...
                    def[x.slots[0].slot.id] = true;
               		...
               		if (!(x.slots[0].slot instanceof Constant)) return true;
                }
...
const chooseReward = i => {
    if (player.choosedRewards.size < getRewardCount()[0] || player.choosedRewards.has(i)) player.choosedRewards.has(i) ? player.choosedRewards.delete(i) : player.choosedRewards.add(i);
};
